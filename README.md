...
On the `auto-text-region` branch, DarkText keeps the framebuffer at its real
DOSBox resolution and no longer assumes a fixed story-pane location. A cheap
pixel detector finds clusters of bright/blue text, selects the strongest text
block, and only that dynamically discovered region is enlarged for RapidOCR.
OCR line boxes are converted back to absolute native framebuffer coordinates,
so temporal menu tracking remains stable even if the discovered crop moves.
If discovery finds no credible block, OCR falls back to the whole native frame
rather than to hard-coded Darklands coordinates.

## Features

- OCR of Darklands narrative text and menu choices
- Native-resolution DOSBox framebuffer processing
- Automatic text-region discovery with no fixed story-pane coordinates
- Fast highlighted-option tracking in absolute framebuffer coordinates
...
One-shot debug captures include the untouched native framebuffer, the detected
text crop, and an annotated full-frame image showing the discovered region and
OCR line boxes.
